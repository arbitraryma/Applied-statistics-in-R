...
## 1.3 Simple Linear regression models
...
X # please notice here, you should also have a column cotains only "1" for the intercept you need to consider
```
Now all you have to do is execute the equation to obtain the coefficients.
```{r}
BETA.HAT <- solve(t(X) %*% X) %*% t(X) %*% Y
BETA.HAT
```

You’ve just used least-squares to fit your model based on the observed data in `demo.data`, which results in the estimates \(\hat{\beta}_0 = 1.225\), \(\hat{\beta}_1 = 1.015\), and \(\hat{\beta}_2 = -0.698\).
...
The other interpretation is similar to the previous model. The interpretation of the log-transformed variable is a bit different. To interpret the slope coefficient we divide it by 100(13.4382/100=0.134382). This tells us that a 1% increase in income increases the dependent variable prestige score by about 0.13 points. This interpretation comes from the fact that a 1% increase in income corresponds to a change of approximately 0.01 in log(income) (since the natural log of a small percentage increase is roughly equal to the percentage itself when expressed as a decimal). Multiplying 0.01 by the coefficient of 13.4382 gives us 0.134.

```{r}
# you can verify by using the income value from the Prestige data
log(Prestige$income[1]*1.01) - log(Prestige$income[1]) # consider to increase 1%
log(Prestige$income[3]*1.01) - log(Prestige$income[3])
```
...
As we show before, the variable `income` seems to disobey the normality assumption, so we can also use the log transformation and then see the changes.
```{r}
model1a <- lm(prestige ~ education + log(income) + type, data = Prestige)
plot(model1a)
```
...
he null hypothesis for the Bonferonni adjusted outlier test is the observation is an outlier. Here observation related to ‘medical.technicians’ is an outlier.
...
If an observation is an outlier and influential (high leverage) then that observation can change the fit of the linear model, it is advisable to remove it. 
...
ancova_model <- aov(exam ~ technique + current_grade +technique:current_grade , data = data)
...
Or you can also use
```{r}
model1.1 <- lm(exam ~ technique + current_grade+technique:current_grade , data = data)
Anova(model1.1)
```

We observe that the interaction is not significant and inconsequence the slopes are not different.
...
We fit a simpler model without interaction.
```{r}
model1.2 <- lm(exam ~ technique + current_grade, data = data)
Anova(model1.2)
```
